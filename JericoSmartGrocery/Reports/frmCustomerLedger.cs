...
namespace JericoSmartGrocery.Reports
{
    public partial class frmCustomerLedger : Form
    {
        ...
        private void printReport()
        {
            ...
            MySqlDataAdapter DA = new MySqlDataAdapter();
            DataSet DS = new DataSet();
            crReports.rptCustomerLedger rptCustomerLedger = new crReports.rptCustomerLedger();
            ...
            try
            {
                ...
                comm.CommandType = CommandType.Text;
                comm.CommandText = "SELECT saleBillNumber, transactiontype, date_format(paymentDate, '%d-%m-%Y') AS paymentDate, (billAmount + otherCharges - discountAmount) as grandTotalAmount, nowPaidAmount, customerAdvanceAmount, balanceAmount FROM tb_salespaymentdetails WHERE customerID ='" + ID + "'and CAST(paymentDate AS DATE) between CAST('" + dtpFrom.Value.ToString("yyyy-MM-dd") + "'AS DATE) and CAST('" + dtpTo.Value.ToString("yyyy-MM-dd") + "'AS DATE);";
                DA = new MySqlDataAdapter(comm);
                DA.Fill(DS, "tb_salespaymentdetails");

                rptCustomerLedger.SetDataSource(DS);

                if (printDialog1.ShowDialog() == DialogResult.OK)
                {
                    printDialog1.AllowSomePages = true;
                    rptCustomerLedger.PrintOptions.PrinterName = printDialog1.PrinterSettings.PrinterName;
                    rptCustomerLedger.PrintToPrinter(printDialog1.PrinterSettings.Copies, false, printDialog1.PrinterSettings.FromPage, printDialog1.PrinterSettings.ToPage);
                }
