...
{
    public partial class frmSupplierPaymentReport : Form
    {
        ...
        public frmSupplierPaymentReport()
        {
            InitializeComponent();
        }

        private void frmSupplierPaymentReport_Load(object sender, EventArgs e)
        {
            lblName.Text = SupName;
        ...
        {
            PaymentMaster.frmSupplierPayment frmSupplierPayment = new PaymentMaster.frmSupplierPayment();
            frmSupplierPayment.ID = ID;
            frmSupplierPayment.SuppName = SupName;
            frmSupplierPayment.balanceAmt = balanceAmt;
            frmSupplierPayment.advanceAmt = advanceAmt;
        ...
        private void dgvPaymentDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 9)
            {
                frmSupplierBillReport frmSupplierBillReport = new frmSupplierBillReport();

                frmSupplierBillReport.billNo = dgvPaymentDetails.SelectedRows[0].Cells[2].Value.ToString();
                frmSupplierBillReport.purchasePayID = dgvPaymentDetails.SelectedRows[0].Cells[1].Value.ToString();
                frmSupplierBillReport.payType = dgvPaymentDetails.SelectedRows[0].Cells[3].Value.ToString();

                frmSupplierBillReport.ShowDialog();
                LoadDatagrid();
            }
        }
