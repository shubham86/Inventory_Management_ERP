...
{
    public partial class frmSupplierPendingPayments : Form
    {
        public frmSupplierPendingPayments()
        {
            InitializeComponent();
        }

        private void frmSupplierPendingPayments_Load(object sender, EventArgs e)
        {
            txtSearch.Focus();
            LoadDatagrid();
        }
        ...
        private void LoadDatagrid()
        {
            ...
            try
            {
                ...
                comm.CommandText = "Proc_Supplier_Pending_Payments";
        ...
        private void searchSupplier()
        {
            ...
            {
                comm.CommandType = CommandType.StoredProcedure;
                comm.CommandText = "Proc_Supplier_Pending_Payments_Search";
        ...
        private Boolean updateStatus(int ID)
        {
            ...
            {
                comm.CommandType = CommandType.StoredProcedure;
                comm.CommandText = "Proc_PurchasePaymentDetails_isSuccessfull";

                comm.Connection = conn;

                comm.Parameters.AddWithValue("purchasePayID", ID);
        ...
        private void dgvpaymentDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 6)
            {
                var confirmResult = MessageBox.Show("Are you sure ? Cheque is cleared.", "Confirm Save !", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (confirmResult == DialogResult.Yes)
                {
                    if (updateStatus(Convert.ToInt32(dgvpaymentDetails.SelectedRows[0].Cells[1].Value)))
                    {
                        successAlert("Cheque cleared successfilly.");
                    }
                    else
                    {
